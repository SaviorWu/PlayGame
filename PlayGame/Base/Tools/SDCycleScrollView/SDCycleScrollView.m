...
- (void)setImageURLStringsGroup:(NSArray *)imageURLStringsGroup
{
    ...
    _totalItemsCount = imageURLStringsGroup.count;
...
- (void)layoutSubviews
{
    ...
    CGRect pageControlFrame = CGRectMake(self.width/2 - size.width/2, y, size.width, size.height);
...
{
//    int theIndex = [self pageControlIndexWithCurrentCellIndex:indexPath.item];
    SDCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
    __block long itemIndex = [self pageControlIndexWithCurrentCellIndex:indexPath.item];
    ...
    __block UIImageView *tapImgV = cell.imageView;
//    WS(weakSelf);
    [cell setImgTapAction:^{
        if ([self.delegate respondsToSelector:@selector(cycleScrollView:view:didSelectItemAtIndex:)]) {
               [self.delegate cycleScrollView:self view:tapImgV didSelectItemAtIndex:itemIndex];
        }else {
            [self collectionView:self.mainView didSelectItemAtIndexPath:[NSIndexPath indexPathForRow:indexPath.row inSection:indexPath.section]];
        }
    }];
