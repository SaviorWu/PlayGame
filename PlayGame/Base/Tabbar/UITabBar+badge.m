...
- (void)bringBadgeToFrontOnItemIndex:(int)index {
    ...
    JTTabBarBtn *badge = [self viewWithTag:tag];
    if (badge) {
...
- (void)showBadgeOnItemIndex:(int)index badgeValue:(int)badgeValue userInteractionEnabled:(BOOL)enable {
    ...
    if (!badge) {
        badge = [[JTTabBarBtn alloc] initWithFrame:decideFrame];
        badge.tag = tag;
        [self addSubview:badge];
    }
...
- (void)deviceOrientationChange {
    if (IsIPad) {
        ...
        for (NSInteger index = 0; index < itemsCount; index++) {
            JTTabBarBtn *badge = [self viewWithTag:index + RCDTabBarButtonTagFrom];
            float peRCDntX = (index + 0.51) / itemsCount;
            CGFloat x = ceilf(peRCDntX * tabFrame.size.width);
            if (index == 0) {
                x = x + 4;
...
- (void)hideBadgeOnItemIndex:(int)index {
    JTTabBarBtn *badge = [self viewWithTag:RCDTabBarButtonTagFrom + index];
    badge.hidden = YES;
}
